...
const SNAKE_WIDTH = 20; // Constant width for all snakes
let camera = { x: 0, y: 0 };
...
function drawSnake(snake, isPlayer) {
    ...
    ctx.beginPath();
    ctx.arc((snake.x - camera.x) * SCALE, (snake.y - camera.y) * SCALE, SNAKE_WIDTH * SCALE / 2, 0, Math.PI * 2);
    ctx.fill();

    for (let segment of snake.segments) {
        ctx.beginPath();
        ctx.arc((segment.x - camera.x) * SCALE, (segment.y - camera.y) * SCALE, SNAKE_WIDTH * SCALE / 2, 0, Math.PI * 2);
        ctx.fill();
    }
}
...
function checkFoodCollision(snake) {
    for (let i = food.length - 1; i >= 0; i--) {
        ...
        let dx = snake.x - f.x;
        let dy = snake.y - f.y;
        if (dx * dx + dy * dy < SNAKE_WIDTH * SNAKE_WIDTH / 4) {
            food.splice(i, 1);
            snake.length += 1;
            generateFood(1);
            return true;
        ...
        for (let j = i + 1; j < ai.length; j++) {
            if (checkSnakeCollision(ai[i], ai[j])) {
                if (ai[i].length > ai[j].length) {
                    ai[i].length += ai[j].length;
                    ai.splice(j, 1);
                    j--; // Adjust index after removal
                    generateAI(1); // Generate a new AI snake to replace the eaten one
                } else {
                    ai[j].length += ai[i].length;
                    ai.splice(i, 1);
